  ...
  <Body>
    <NotifyEventUpdated>
      <event>
        ...
        <ResolutionCodeId>999</ResolutionCodeId>
      </event>
    </NotifyEventUpdated>
  </Body>
</Envelope>
EOF
        ...
        subtest 'A report sent to Confirm, then redirected to Echo' => sub {
            ...
            is $report->get_extra_field_value('HandoverNotes'), 'This is a handover note';
            is $report->category, 'Referred to Veolia';
        };
        subtest '...then redirected back to Confirm' => sub {
            $report->update({ external_id => 'guid' });
            # XXX set whensent here
            ...
            is $report->category, 'Referred to Bromley Streets';
            # Need to check whensent still set
            # and need to check newly submitted update - and hide it?
        };
